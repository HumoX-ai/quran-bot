...
async function fetchQoranData() {
    try {
        const response = await axios_1.default.get("https://cdn.jsdelivr.net/gh/fawazahmed0/quran-api@1/editions/uzb-muhammadsodikmu.json");
        ...
        console.log("Qur'on ma'lumotlari yuklandi. Birinchi element:", quranData[0]);
    }
    catch (error) {
...
bot.callbackQuery(/^select_surah_(\d+)$/, async (ctx) => {
    try {
        ...
        const keyboard = new grammy_1.InlineKeyboard();
        const currentSurahs = utils_1.surahNames.slice(start, end);
        ...
        if (end < utils_1.surahNames.length) {
...
bot.callbackQuery(/^surah_(\d+)$/, async (ctx) => {
    try {
        ...
        for (let i = 0; i < Math.min(verses.length, 20); i += 5) {
            ...
            row.forEach((verse) => {
                keyboard.text(verse.verse.toString(), `verse_${surahNumber}_${verse.verse}`);
            });
            keyboard.row();
        }
        if (verses.length > 20) {
        ...
        await ctx.editMessageText(`*${surahNumber}-сура, оятни танланг:*\n` +
...
bot.callbackQuery(/^more_verses_(\d+)_(\d+)$/, async (ctx) => {
    try {
        ...
        for (let i = startVerse; i < Math.min(verses.length, startVerse + 20); i += 5) {
        ...
        if (startVerse + 20 < verses.length) {
            keyboard.text("Кўпроқ оятлар ➡️", `more_verses_${surahNumber}_${startVerse + 20}`);
        }
        // Orqaga qaytish tugmasi
        keyboard
            .row()
            .text("⬅️ Олдинги", `more_verses_${surahNumber}_${Math.max(0, startVerse - 20)}`);
        keyboard.text("🔙 Суралар рўйхати", "select_surah_1");
        await ctx.editMessageText(`*${surahNumber}-сура, оятни танланг:*\n` +
            `_Жами оятлар сони: ${verses.length}_\n` +
            `_Ҳозирги оятлар: ${startVerse + 1}-${Math.min(startVerse + 20, verses.length)}_`, {
            parse_mode: "Markdown",
            reply_markup: keyboard,
        });
    }
...
bot.callbackQuery(/^verse_(\d+)_(\d+)$/, async (ctx) => {
    try {
        ...
        const verse = quranData.find((v) => v.chapter === surahNumber && v.verse === verseNumber);
        if (verse) {
            const keyboard = new grammy_1.InlineKeyboard();
            if (verseNumber > 1) {
                keyboard.text("⬅️ Олдинги оят", `verse_${surahNumber}_${verseNumber - 1}`);
            }
            const nextVerse = quranData.find((v) => v.chapter === surahNumber && v.verse === verseNumber + 1);
            if (nextVerse) {
                keyboard.text("Кейинги оят ➡️", `verse_${surahNumber}_${verseNumber + 1}`);
            }
            keyboard.row().text("🔙 Оятлар рўйхати", `surah_${surahNumber}`);
            await ctx.editMessageText(`*${surahNumber}-сура, ${verseNumber}-оят*\n\n` +
                `${verse.text}\n\n` +
...
bot.callbackQuery("about", async (ctx) => {
    const keyboard = new grammy_1.InlineKeyboard().text("🔙 Асосий меню", "select_surah_1");
    await ctx.editMessageText("*Қуръони Карим бўйича қидирув боти*\n\n" +
